...
- **FilterChipGroup** — multi-select chip set per facet (platform / genre / status / whose-pick). *Props: `facet`, `options`, `selected[]`, `onChange`.* A11y: `role="group"` with `aria-label` naming the facet.
