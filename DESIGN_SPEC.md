...
- **InProgressEntry actions** — on its own row beneath the season stepper / state toggle / "Finished it", on the `/in-progress` page and the dashboard's Watching/Paused cards.
- **Every other dashboard card** — want-to-watch / completed / dropped `WatchEntryCard`s render the toggle standalone (those have no `InProgressActions`), so a show can be marked co-watched at any status without first reverting it to Watching.
...
4. **IdentityChip** — `h-11 w-11` colored monogram tile (border hairline, no strong border). Dropdown menu (Radix `DropdownMenu`) carries **Settings**, **Switch user** (opens the passcode dialog), **Log out**. Settings has no dedicated app-bar icon — it's low-traffic and lives in this menu.
